...
export class VoiceXpService {
    static instance;
    intervals = new Map(); // key: guildId:discordId
    constructor() { }
    static getInstance() {
    ...
    buildKey(guildId, discordId) {
        return `${guildId}:${discordId}`;
    }
    isTracking(guildId, discordId) {
        return this.intervals.has(this.buildKey(guildId, discordId));
    }
    startTracking(guildId, discordId, client) {
    ...
    stopTracking(guildId, discordId) {
        const key = this.buildKey(guildId, discordId);
     ...
     */
    async refreshChannel(channel, client) {
