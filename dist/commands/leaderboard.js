import { SlashCommandBuilder, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, InteractionContextType, ComponentType, } from 'discord.js';
...
export const data = new SlashCommandBuilder()
    ...
    .setRequired(false)
    .setMinValue(1));
async function buildLeaderboardEmbed(guildId, guildName, guildIcon, page, callerId) {
...
function buildButtons(page, totalPages, disabled = false) {
    return new ActionRowBuilder().addComponents(new ButtonBuilder()
        ...
        .setEmoji('▶️')
        .setStyle(ButtonStyle.Primary)
        .setDisabled(disabled || page >= totalPages), new ButtonBuilder()
        .setCustomId('lb_last')
        .setEmoji('⏭️')
        .setStyle(ButtonStyle.Secondary)
...
export async function execute(interaction) {
    ...
    try {
        let { embed, safePage, totalPages } = await buildLeaderboardEmbed(interaction.guild.id, interaction.guild.name, guildIcon, startPage, interaction.user.id);
        ...
        collector.on('collect', async (i) => {
            ...
            const result = await buildLeaderboardEmbed(interaction.guild.id, interaction.guild.name, guildIcon, currentPage, interaction.user.id);
