...
export const data = new SlashCommandBuilder()
    ...
    .setName("removerole")
    .setDescription("Retirer le rôle lorsque l'utilisateur retire sa réaction"))
    .addStringOption((o) => o
    .setName("messageid")
...
function parseEmoji(input) {
    ...
    if (match)
        return { type: "custom", id: match[1], raw: input };
    return { type: "unicode", raw: input };
}
export async function execute(interaction) {
    ...
    try {
        ...
        if (parsed.type === "custom") {
            ...
            reactToken = emojiObj.toString();
        }
        // Vérification si la réaction existe déjà
        const existingReaction = targetMessage.reactions.cache.find((r) => r.emoji.id ? r.emoji.id === parsed.id : r.emoji.name === parsed.raw);
        if (existingReaction) {
        ...
        await interaction.editReply(`Rôle de réaction ajouté. Lien: https://discord.com/channels/${interaction.guildId}/${interaction.channelId}/${targetMessage.id}`);
    }
    catch (err) {
